...
//					}
	...
	public List<Dominance> getDominance(String symbol) {
		List<Dominance> dominances = new ArrayList<>();
		Calendar today = Calendar.getInstance();
		today.set(Calendar.HOUR_OF_DAY, 0);
		today.set(Calendar.MINUTE, 0);
...
//			System.out.println("currentDayTimeMillis:" + currentDayTimeMillis); 
		...
		for (CandlesticksCache _1hCandlesticksCache : _1hCandlesticksCaches) {
			...
			String marketSymbol = _1hCandlesticksCache.getSymbol();
			// Only check for btc or eth market
			if (!(marketSymbol.endsWith("BTC") || marketSymbol.endsWith("ETH"))) {
				continue;
			}
			
			if (!(marketSymbol.substring(0, marketSymbol.length() - "BTC".length()).equalsIgnoreCase(symbol))) {
				continue;
			}
			
			System.out.println("getDominance(" + symbol + ")");
